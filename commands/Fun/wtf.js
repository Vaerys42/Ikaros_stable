...
module.exports = class Wtf extends commando.Command {
	constructor(client) {
		super(client, {
			name: 'wtf',
			group: 'fun',
			memberName: 'wtf',
			description: "Puissiez vous être guidé sur la voie du WTF",
			details: "Utilisez la commande `?wtf`",
			examples: ["```?wtf```"]
		});
	}
	...
	async run(msg, args){
		...
		let message_list = messages.findAll('content', 'wtf');
